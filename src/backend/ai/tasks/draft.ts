...
export async function draft(
  ...
  enforceTokenLimit(context, 120000, "Drafting Context");

  // Fetch active bullets and build the context block
  const bulletsBlock = await buildBulletsContext(env);

  const systemPrompt = `You are Colby, a precise career assistant. Draft polished, truthful job-application content from the provided context.
${bulletsBlock ? `\n${bulletsBlock}` : ""}`;
