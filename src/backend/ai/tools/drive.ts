...
export class GoogleDriveClient {
  ...
  /**
   * Upload an arbitrary file into Google Drive via multipart upload.
   *
   * Unlike `createDocFromHtml`, this preserves the requested MIME type. It is
   * used for raw role artifacts such as markdown, HTML snapshots, PDFs, and
   * downloaded NotebookLM podcast MP3s.
   */
  async uploadFile(
    name: string,
    parentFolderIdInput: string | undefined,
    bytes: ArrayBuffer | Uint8Array,
    mimeType: string,
  ): Promise<{ id: string; name: string; webViewLink?: string }> {
    const parentFolderId = parentFolderIdInput ? extractGoogleId(parentFolderIdInput) : undefined;
    const boundary = `-------core-resumes-${crypto.randomUUID()}`;
    ...
    const body = new Uint8Array(prefix.length + data.length + suffix.length);
    body.set(prefix, 0);
    body.set(data, prefix.length);
    body.set(suffix, prefix.length + data.length);
    ...
    if (!response.ok) {
      throw new Error(
        `Google Drive file upload failed: ${response.status} ${await response.text()}`,
      );
    }
  ...
  /**
   * Create a native Google Doc from plain text.
   *
   * Drive's HTML-to-Doc conversion gives us a real editable Doc while keeping
   * transcript creation simple and free from template assumptions.
   */
  async createDocFromText(
    name: string,
    parentFolderIdInput: string,
    plainText: string,
  ): Promise<{ id: string; name: string; webViewLink?: string }> {
    const html = `<html><body><pre style="white-space: pre-wrap; font-family: Arial, sans-serif;">${escapeHtml(plainText)}</pre></body></html>`;
    return this.createDocFromHtml(name, html, parentFolderIdInput);
  }
...
/** Escape text before embedding it in a minimal HTML document for Drive import. */
