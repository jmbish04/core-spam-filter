  ...
  async invokeStructured<TInput, TOutput>(
      ...
      if (choice && isRecord(choice.message)) {
        ...
        if (typeof content === "string") {
          ...
          if (typeof parsed !== "object" || parsed === null) {
            throw new Error(
              `WorkersAIProvider: JSON parsed to non-object: ${JSON.stringify(parsed)} from string: ${content}`,
            );
          }
          return parsed;
        }
    ...
    if (isRecord(result)) {
      ...
      if (typeof textValue === "string") {
        const parsed = JSON.parse(stripJsonFences(textValue));
        if (typeof parsed !== "object" || parsed === null) {
          throw new Error(
            `WorkersAIProvider: JSON parsed to non-object: ${JSON.stringify(parsed)} from string: ${textValue}`,
          );
        }
        return parsed;
      }
