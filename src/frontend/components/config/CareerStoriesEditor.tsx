...
export function CareerStoriesEditor() {
  const [value, setValue] = useState("");
  const [isDefault, setIsDefault] = useState(false);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    readConfig<string>("career_stories", "").then((result) => {
  ...
  async function save() {
    ...
    try {
      await apiPut("/api/config/career_stories", {
        value: value,
      });
  ...
  return (
    <Card className="rounded-lg">
      <CardHeader>
        <CardTitle>Career Stories</CardTitle>
        <CardDescription>
          Personal narratives and foundational stories injected into the job analysis prompt.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3">
        {isDefault && (
          <div className="flex items-start gap-2 rounded-md border border-amber-500/20 bg-amber-500/5 px-3 py-2 text-sm text-amber-400">
            ...
            <span>Using default fallback value. Save custom career stories to override.</span>
          </div>
        )}
